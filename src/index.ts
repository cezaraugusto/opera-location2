...
export default function locateOpera (
...
const DEFAULT_INSTALL_STEPS: InstallGuidanceStep[] = [
  {
    summary: 'Install Opera from the official site',
...
export function getInstallGuidance (opts?: InstallGuidanceOptions): string {
  ...
  const lines = [
    "We couldn't find an Opera browser on this machine.",
    '',
    'To install one:',
    ''
  ]
  ...
  lines.push('')
  lines.push('Alternatively, set OPERA_BINARY=/path/to/opera and re-run.')

  return lines.join('\n')
}

export function locateOperaOrExplain (
  options?: boolean | {allowFallback?: boolean}
): string {
  const allowFallback =
    typeof options === 'boolean' ? options : Boolean(options?.allowFallback)

  const found = locateOpera(allowFallback) || locateOpera(true)
...
/**
 * Cross-platform Opera version resolver.
